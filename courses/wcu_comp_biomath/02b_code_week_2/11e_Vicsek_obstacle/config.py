...
class VicsekConfig:
    ...
    Attributes:
        ...
        obstacle_center: Center of circular obstacle (x, y) or None
    ...
    obstacle_center: Optional[tuple[float, float]] = None
    ...
    def __post_init__(self):
        ...
        assert self.obstacle_radius > 0, "Obstacle radius must be positive"
        
        if self.obstacle_center is not None:
            assert len(self.obstacle_center) == 2, "Obstacle center must be (x, y)"
            assert 0 <= self.obstacle_center[0] <= self.L, "Obstacle x must be in domain"
            assert 0 <= self.obstacle_center[1] <= self.L, "Obstacle y must be in domain"