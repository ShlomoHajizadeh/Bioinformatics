...
from obstacle import apply_obstacle_avoidance
...
def update_headings(headings: np.ndarray, positions: np.ndarray, 
    ...
    """Update particle headings according to Vicsek dynamics with obstacle avoidance.
    
    θ_i(t_{n+1}) = E(θ_{i,r}(t_n)) + η ξ_n
    
    If obstacle is nearby, apply avoidance maneuver instead.
    ...
    # Compute new headings based on Vicsek rule
    ...
    # Apply obstacle avoidance (overrides Vicsek rule for particles near obstacle)
    new_headings = apply_obstacle_avoidance(new_headings, positions, config)
