...
def initialize_agents(N, fish_count, shark_count):
    ...
    Returns:
        fish (list): List of fish agents, each represented as a tuple (x, y, direction).
        sharks (list): List of shark agents, each represented as a tuple (x, y, direction).
    """
    ...
    fish = [(i % N, i // N, np.random.randint(1, 5)) for i in fish_indices]

    # Assign the next shark_count indices to sharks
    shark_indices = indices[fish_count:fish_count + shark_count]
    sharks = [(i % N, i // N, np.random.randint(1, 5)) for i in shark_indices]
