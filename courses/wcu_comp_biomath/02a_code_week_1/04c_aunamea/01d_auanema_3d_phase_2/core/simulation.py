...
def initialize_population(config, rng):
    ...
    initial_age = config.JUVENILE_AGE_THRESHOLD + 0.5
...
def run_simulation(config):
    ...
    for step in range(config.N_STEPS):
        ...
        # ---------------------------------
        # Agent-environment and agent-agent interactions
        # ---------------------------------
        ...
        # Reproduction
        # ---------------------------------
        births, next_id = process_reproduction(
            population,
            environment,
            config,
            next_id,
        ...
        if config.SAVE_ANIMATION and step % config.ANIMATION_EVERY_N_STEPS == 0:
            ...
            nutrient_history.append(environment.nutrients.copy())
            pheromone_history.append(environment.pheromones.copy())
