"""
Simulation class for Version B.
"""
...
from core.interactions import step_version_b
...
class Simulation:
    ...
    def validate_state(self):
        ...
        assert np.all((0.0 <= defense_level) & (defense_level <= self.params.DEFENSE_MAX)), \
    ...
    def step(self):
        self.state = step_version_b(self.state, self.params)
        self.validate_state()
        self.record_history()
        ...
        if self.params.DEBUG and self.state["step"] % self.params.PRINT_EVERY == 0:
            print(
                ...
                f"attacked frac = {self.state['attacked'].mean():.3f} | "
                f"mean defense = {self.state['defense_level'].mean():.3f} | "
                f"day bloom frac = {((self.state['bloom_mode'] == self.params.BLOOM_DAY).mean()):.3f}"
            )
