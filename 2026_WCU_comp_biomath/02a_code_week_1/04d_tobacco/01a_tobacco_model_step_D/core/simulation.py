"""
Simulation class for Version D.
"""
...
from core.grid import (
    ...
    initialize_signal,
    initialize_priming,
    initialize_predator_attractant,
    initialize_growth_inhibitor,
    initialize_egg_queues,
    initialize_caterpillars,
    random_positions,
)
from core.interactions import step_version_d
...
class Simulation:
    def __init__(self, params):
        ...
        self.state = {
            ...
            "defense_level": initialize_defense_level(params.GRID_HEIGHT, params.GRID_WIDTH),
            ...
            "predator_attractant": initialize_predator_attractant(
                params.GRID_HEIGHT, params.GRID_WIDTH
            ),
            "growth_inhibitor": initialize_growth_inhibitor(
                params.GRID_HEIGHT, params.GRID_WIDTH
            ),
        ...
        self.history = {
            ...
            "mean_signal": [],
            "mean_priming": [],
            "mean_predator_attractant": [],
            "mean_growth_inhibitor": [],
    ...
    def validate_state(self):
        ...
        predator_attractant = self.state["predator_attractant"]
        growth_inhibitor = self.state["growth_inhibitor"]
        ...
        assert defense_level.shape == (h, w)
        assert signal.shape == (h, w)
        assert priming.shape == (h, w)
        assert predator_attractant.shape == (h, w)
        assert growth_inhibitor.shape == (h, w)
        assert caterpillars.shape == (h, w)
        assert eggs_queue.shape[1:] == (h, w)

        assert np.all(np.isfinite(plant_health))
        assert np.all(np.isfinite(defense_level))
        assert np.all(np.isfinite(signal))
        assert np.all(np.isfinite(priming))
        assert np.all(np.isfinite(predator_attractant))
        assert np.all(np.isfinite(growth_inhibitor))
        ...
        assert np.all((0.0 <= signal) & (signal <= self.params.SIGNAL_MAX))
        assert np.all((0.0 <= priming) & (priming <= self.params.PRIMING_MAX))
        assert np.all((0.0 <= predator_attractant) & (predator_attractant <= self.params.ATTRACTANT_MAX))
        assert np.all((0.0 <= growth_inhibitor) & (growth_inhibitor <= self.params.GROWTH_INHIBITOR_MAX))
        assert np.all(caterpillars >= 0)
        assert np.all(eggs_queue >= 0)

        assert np.all((0 <= moth_x) & (moth_x < w))
        assert np.all((0 <= moth_y) & (moth_y < h))
        assert np.all((0 <= pred_x) & (pred_x < w))
        assert np.all((0 <= pred_y) & (pred_y < h))
    ...
    def record_history(self):
        ...
        self.history["mean_predator_attractant"].append(
            self.state["predator_attractant"].mean()
        )
        self.history["mean_growth_inhibitor"].append(
            self.state["growth_inhibitor"].mean()
        )
    ...
    def capture_frame(self):
        self.frames.append({
            ...
            "predator_attractant": self.state["predator_attractant"].copy(),
            "growth_inhibitor": self.state["growth_inhibitor"].copy(),
    ...
    def step(self):
        self.state = step_version_d(self.state, self.params)
        self.validate_state()
        self.record_history()
        ...
        if self.params.DEBUG and self.state["step"] % self.params.PRINT_EVERY == 0:
            print(
                ...
                f"mean attractant = {self.state['predator_attractant'].mean():.3f} | "
                f"mean inhibitor = {self.state['growth_inhibitor'].mean():.3f}"
            )
