...
from visualization import plot_state, create_animation, create_simple_animation
...
def plot_trajectory_summary(trajectory: dict, config: VicsekConfig, 
    ...
    plot_state(trajectory['positions'][0], trajectory['headings'][0], 
               config, ax=ax1)
    ax1.set_title('Initial State (t=0)', fontsize=14, fontweight='bold')
    ...
    ax3.set_title('Order Parameter Evolution', fontsize=14, fontweight='bold')
    ...
    ax4.set_title('Final Heading Distribution', fontsize=14, fontweight='bold')
...
def main():
    ...
    # Example 2: With obstacle in center
    config = VicsekConfig(
        N=300,                      # Number of particles
        L=10.0,                     # Domain size
        r=1.0,                      # Interaction radius
        ...
        n_steps=500,                # Number of simulation steps
        seed=42,                    # Random seed for reproducibility
        obstacle_center=(5.0, 5.0), # Obstacle at center of domain
        obstacle_radius=0.8         # Obstacle radius
    )
    ...
    if config.obstacle_center is not None:
        print(f"  obstacle_center:      ({config.obstacle_center[0]:.1f}, "
              f"{config.obstacle_center[1]:.1f})")
        print(f"  obstacle_radius:      {config.obstacle_radius:.2f}")
    else:
    ...
    # Create full animation with order parameter plot
    anim = create_animation(
        trajectory, 
        config,
        interval=50,           # 50ms between frames
        skip_frames=2,         # Show every 2nd frame for smoother/faster animation
        save_path=output_dir / "vicsek_animation.gif",
        show=True
    )
    
    # Optionally create simple animation (particles only)
    # Uncomment the following lines if you want a simpler version too:
    # print("\nGenerating simple animation...")
    # anim_simple = create_simple_animation(
    #     trajectory,
    #     config,
    #     interval=50,
    #     skip_frames=2,
    #     save_path=output_dir / "vicsek_simple.gif",
    #     show=False
    # )
    ...
    if config.obstacle_center is not None:
        print("\nNote: Particles avoid the obstacle using perpendicular avoidance.")
