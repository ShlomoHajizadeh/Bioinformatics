...
def plot_landing_analysis(times, diagnostics):
    ...
    # Infer N from maximum count
    N_total = 50  # Default from parameters
...
def plot_rolling_analysis(times, diagnostics):
    """
    Plot analysis specific to rolling behavior (Stage 3).
    ...
    # Plot 1: Front and rear position over time
    axes[0, 0].plot(times, diagnostics['front_position'], 'b-', linewidth=2, label='Front')
    axes[0, 0].plot(times, diagnostics['rear_position'], 'r-', linewidth=2, label='Rear')
    axes[0, 0].plot(times, diagnostics['center_x'], 'g--', linewidth=2, label='Center')
    axes[0, 0].set_xlabel('Time', fontsize=12)
    axes[0, 0].set_ylabel('Horizontal Position', fontsize=12)
    axes[0, 0].set_title('Swarm Marching Progress', fontsize=13, fontweight='bold')
    axes[0, 0].legend(fontsize=11)
    axes[0, 0].grid(True, alpha=0.3)
    
    # Plot 2: Swarm length over time
    axes[0, 1].plot(times, diagnostics['swarm_length'], 'purple', linewidth=2)
    axes[0, 1].set_xlabel('Time', fontsize=12)
    axes[0, 1].set_ylabel('Swarm Length', fontsize=12)
    axes[0, 1].set_title('Swarm Elongation', fontsize=13, fontweight='bold')
    axes[0, 1].grid(True, alpha=0.3)
    
    # Plot 3: Rolling speed over time
    axes[1, 0].plot(times, diagnostics['mean_rolling_speed'], 'orange', linewidth=2, label='Rolling Speed')
    axes[1, 0].plot(times, diagnostics['mean_horizontal_velocity'], 'cyan', linewidth=2, label='Mean Horizontal Velocity')
    axes[1, 0].set_xlabel('Time', fontsize=12)
    axes[1, 0].set_ylabel('Velocity', fontsize=12)
    axes[1, 0].set_title('Rolling Dynamics', fontsize=13, fontweight='bold')
    axes[1, 0].legend(fontsize=11)
    axes[1, 0].grid(True, alpha=0.3)
    axes[1, 0].axhline(y=0, color='black', linestyle='--', linewidth=1, alpha=0.3)
    
    # Plot 4: Rolling count over time
    axes[1, 1].plot(times, diagnostics['rolling_count'], 'brown', linewidth=2)
    axes[1, 1].set_xlabel('Time', fontsize=12)
    axes[1, 1].set_ylabel('Number Rolling', fontsize=12)
    axes[1, 1].set_title('Active Rolling Locusts', fontsize=13, fontweight='bold')
    axes[1, 1].grid(True, alpha=0.3)
